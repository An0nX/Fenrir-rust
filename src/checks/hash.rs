...
mod sha256_impl {
    ...
    #[inline(always)]
    fn usigma0(x: u32) -> u32 { x.rotate_right(7) ^ x.rotate_right(18) ^ (x >> 3) }
    #[inline(always)]
    fn usigma1(x: u32) -> u32 { x.rotate_right(17) ^ x.rotate_right(19) ^ (x >> 10) }
    #[inline(always)]
        ...
        pub fn finalize(mut self) -> [u8; 32] {
         ...
         fn process_block(&mut self, block: &[u8]) {
             ...
             for i in 0..64 {
                 let s1 = sigma1(e);
                 let ch_val = ch(e, f, g);
                 let temp1 = h.wrapping_add(s1).wrapping_add(ch_val).wrapping_add(K[i]).wrapping_add(w[i]);
                 let s0 = sigma0(a);
