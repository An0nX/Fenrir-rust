...
pub fn scan_filesystem(config: &Config, iocs: &IocCollection) -> Result<()> {
    ...
    let forced_string_dirs_paths: Vec<&Path> = config
...
fn process_file(
    ...
    if excluded_dirs
        .iter()
        .any(|ex_dir| file_path.starts_with(ex_dir))
    {
    ...
    let metadata = match fs::metadata(file_path) {
        Ok(md) => md,
        Err(e) => {
            return Err(FenrirError::FileAccess {
    ...
    if config.check_only_relevant_extensions && !is_elf {
        match extension_lower.as_deref() {
            ...
            _ => {
                log_debug!(
                    config,
                    "Deactivating string/hash checks on {} due to irrelevant extension.",
                    file_path.display()
                );
    ...
    if file_size_kb > config.max_file_size_kb {
        log_debug!(
            config,
            "Deactivating string/hash checks on {} due to size ({} KB > {} KB)",
            file_path.display(),
            file_size_kb,
    ...
    // Отформатировано
    if !do_string_check
        && config.enable_string_check
        && forced_string_dirs
            .iter()
            .any(|forced_path| file_path.starts_with(forced_path) || file_path == *forced_path)
    {
        log_debug!(
            config,
            "Activating string check on {} due to forced directory/path.",
            file_path.display()
        );
    ...
    if do_date_check {
        if let Err(e) = timeframe::check_timeframe(file_path, config) {
            log_debug!(
                config,
                "Error during timeframe check for {}: {}",
                file_path.display(),
                e
            );
        }
